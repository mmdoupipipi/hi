...
#        print "from ", socket.inet_ntoa(src),":",sport, " to ", socket.inet_ntoa(dst),":",dport
        if dport == 53 :
            ...
            print '%d.%d.%d.%d:'% tuple(map(ord, list(src))) , "DNS请求域名: ", dns.qd[0].name, "\tDNS请求ID号: ", dns.id, "\tDNS QR是:", dns.qr, "\t查询类型是:", type_table[dns.qd[0].type]
#            print "dns.qd is ", dns.qd
        elif sport == 53 :
            # UDP/53 is a DNS response
            dns = dpkt.dns.DNS(data)
            print "\n获取到给%d.%d.%d.%d的DNS响应:"% tuple(map(ord, list(src))),"\nDNS ID是:", dns.id,"\t请求域名：", dns.qd                
